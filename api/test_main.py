...
def test_predict_close_price():
    response = client.get("/predict", auth=("hamza", "ennaji"))
...
def test_get_price_history():
    ...
    interval = "1H"
    response = client.get(f"/price_history/{start_time}/{end_time}/{interval}/BTC", auth=("hamza", "ennaji"))
    assert response.status_code == 200
    assert len(response.json()) > 0

def test_add_user():
    new_user = {"username": "test_user", "password": "test_password"}
    response = client.post("/add_user", json=new_user, auth=("hamza", "ennaji"))
    assert response.status_code == 200
    assert "user_id" in response.json()

def test_get_model_performance():
    response = client.get("/model_performance", auth=("hamza", "ennaji"))
    assert response.status_code == 200
